 ...
 * Character-specific offsets table.
 */
export type WearableOffsets = Record<
    string,
    Partial<Record<"front" | "back" | "side", WearableOffset>> | WearableOffset
>;
/**
 * Anchor definition — a point projected from the sprite's origin+angle.
 * Used for line-attach points like rod tips.
 ...
 * Fallback chain when something's missing: walk → still → idle; idle → still; still → idle.
 */
export type AnimationSet = Partial<
    Record<Direction, {
        still?: string;
        idle?: string;
        walk?: string;
 ...
 * @returns {any}
 */
export function createChunkyButton<T extends import("phaser").Scene>(
    scene: T,
    x: number,
    y: number,
    width: number,
 ...
 */
export function loadSpritesheetOnce(
    scene: Phaser.Scene,
    key: string,
    url: string,
 ...
 */
export function bakeCircularCrop(
    scene: import("phaser").Scene,
    sourceKey: string,
    frameNameOrIndex: string | number,
    outKey: string,
    circle: {
        cx: number;
        cy: number;
 ...
 * Default when no callback is provided: no-op (`"ignored"`).
 */
